...
line_cache = {
    (length, angle, color): pygame.transform.rotate(LOGIC.Tool.draw_rectangle((3, 495) if length == 500 else (2, length), 0, color).convert_alpha(), angle)
...
particle_cache = {
    f"{(10, 10)}_{color_dict[5]}": LOGIC.Tool.draw_rectangle((10, 10), 0, color_dict[5]).convert(),
    **{f"{(3 * i, 3 * i)}_{color_dict[6]}": LOGIC.Tool.draw_rectangle((3 * i, 3 * i), 0, color_dict[6]).convert() for i in range(1, 5)},
    **{f"{(2, 2)}_{color_dict[i]}": LOGIC.Tool.draw_rectangle((2, 2), 0, color_dict[i]).convert() for i in range(1, 7)}
}
