...
text_cache = {
    **{(2 ** i, color_dict[6]): font.render(f"+ {2 ** i}", False, color_dict[6]).convert_alpha() for i in range(1, 15)},
    **{(2 ** i, color_dict[7]): font.render(f"+ {2 ** i}", False, color_dict[7]).convert_alpha() for i in range(1, 15)},
