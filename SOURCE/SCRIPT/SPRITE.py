...
from random import choice, randint, sample, random, uniform
...
def circle_barrage(type: int, color: list, spawn_pos: tuple, locate: tuple, group: pg.sprite.Group):
    ...
    Barrage(effective, type, 3, color[0], direct(-delta[0], -delta[1]), spawn_pos, 0, barrage_cache[(type, color[0])], group, 3)
...
def polygon_barrage(type: int, color: list, spawn_pos: tuple, locate: tuple, group: pg.sprite.Group):
    for i in range(locate[0] - 32, locate[0] + 33, 64):
        ...
        Barrage(effective, type, 3, color[0], direct(-delta[0], -delta[1]), spawn_pos, 0, barrage_cache[(type, color[0])], group, 2)
...
def point_barrage(type: int, color: list, locate: tuple, group: pg.sprite.Group):
    for _ in range(3):
        ...
        Barrage(effective, type, 4, color[0], direct(-delta[0], -delta[1]), pos, 0, barrage_cache[(type, color[0])], group, 3)
