...
from LOGIC.TOOL import vector, add, draw_rectangle
...
class Hro(pygame.sprite.Sprite):
        ...
        if th.bullet_counter < 18:
            for bullet_info in bullet_type:
                ...
                for j in range(45, 136, 90):
                    ...
                    current_angle = degrees(atan) + j + th.bullet_delay
                    sprite_pos = (current_pos.x, current_pos.y)
                    sprite = Barrage(effective, 0, 4, th.color, current_angle, sprite_pos, barrage_cache[f"0_{th.color}"])
        ...
        if th.bullet_counter < 1:
            for _ in range(6):
                for j in (150, 185, 220, 255, 292, 327, 365, 400, 435):
                    for k in range(1, 4):
                        sprite_pos = (j, 60)
                        two_point = add((th.target_pos[0], th.target_pos[1] - 96), (-sprite_pos[0], -sprite_pos[1]))
                        atan2_ = atan2(-two_point[0], -two_point[1])
                        current_angle = degrees(atan2_) * k
                        sprite = Barrage(effective, 0, speed, th.color, current_angle, sprite_pos, barrage_cache[f"0_{th.color}"])
    ...
    def fire(th) -> None:
        ...
        if th.bullet_timer % 6 == 0 and th.bullet_counter < 3:
            ...
            for i in range(-30, 31, 30):
                two_point = add((th.target_pos[0], th.target_pos[1]), (-pos[0], -pos[1]))
                atan2_ = atan2(-two_point[0], -two_point[1])
                current_angle = degrees(atan2_) + i
                sprite = Barrage(effective, 0, 4, th.color, current_angle, pos, barrage_cache[f"0_{th.color}"])
    ...
    def update(th) -> None:
        ...
        if th.timer % 110 == 0:
            th.x, th.y = choice((150, 220, 292, 365)), choice((60, 120, 180, 240))
            th.bullet_counter = 0
            th.bullet_delay = 0
            th.timer = 0
            th.is_choose = False
        ...
        if th.timer % 110 >= 91:
            ...
            if th.timer % 91 == 0:
                for i in range(0, 360, 120 if th.choice == th.fire else 90):
                    ...
                    particle = Barrage(effective, 0, 4, color_dict[6], current_angle, pos, barrage_cache[f"0_{color_dict[6]}"], False)
        ...
        th.rect.center = vector(th.rect.center, (th.x, th.y), 5)[0]
