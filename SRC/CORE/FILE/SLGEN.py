...
class SLGenerator: # 关卡生成器
    ...
    def ld_stg(th):
        ...
        th.row = 0
        # 读取关卡文件
        filename = f'AST/STG_{th.own.stg_mgr.stg}-{th.own.stg_mgr.lv}.stg'
        # 加载关卡文件
                ...
                for i in range(col - 1): # 遍历字符
                    if line[i] != 'o': # 如果字符不是o
                        ...
                        clr_dict = { # 砖块颜色字典
                            ...
                            2: (255, 255, 0), # 第二关（黄色
                            3: (0, 255, 0), # 第三关（绿色
                            4: (128, 0, 128), # Extra关（紫色
                            5: (255, 255, 255) # 通用（白色
                        }
                        # 砖块厚度
                        bd = rand.choice(bd_id) if rand.random() < 0.1 * th.own.stg_mgr.stg else 2
                        # 否则按照关卡数生成对应砖块
    ...
    def lgc(th): # 逻辑
        ...
        if (th.own.stg_mgr.lv == 6 and
            not th.own.stg_mgr.is_spwn_fri):
