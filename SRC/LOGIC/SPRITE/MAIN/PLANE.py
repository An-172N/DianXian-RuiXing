...
from BRICK.HUMAN.MAINCHAR import MainChar
...
class Plane:
    ...
    def choose_pln(th):
        pln_dict = {
            1: ('Kli', 'AST\IMG_KLI.png'),
            2: ('Kli', 'AST\IMG_KLI.png'),
            3: ('Kli', 'AST\IMG_KLI.png'),
            4: ('Hro', 'AST\IMG_HRO.png')
        }

        if th.own.stg_mgr.stg in pln_dict:
            name, pic = pln_dict[th.own.stg_mgr.stg]
            th.char = MainChar(name, pic)

        th.spwn_pln()

    def pln_spd(th):
        if isinstance(th.char, MainChar):
            if th.is_slow:
                return 1 if th.char.name == 'Kli' else 3
            else:
                return 4 if th.char.name == 'Kli' else 5

    def upd_pos(th):
        if th.mv_right:
            th.char.rect.x += th.pln_spd()
        if th.mv_left:
            th.char.rect.x -= th.pln_spd()
