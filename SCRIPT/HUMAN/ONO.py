...
from SCRIPT import SPRITE, TOOL


class Ono(pygame.sprite.Sprite):
    asset_path = os.path.join(os.path.dirname(os.path.abspath((__file__))), '..\..\ASSET')
    char_image = pygame.image.load(os.path.join(asset_path, 'IMAGE\IMG_ONO.png')).convert_alpha()
    ...
    def free(th) -> None:
        ...
        if th.bullet_timer % 1 == 0 and th.bullet_counter < 8:
            ...
            for i, j in itertools.product(
                ...
                sprite = SPRITE.Barrage(2, 3.5, th.color, j, (x, y))
                sprite.update()
    ...
    def fire(th) -> None:
        if th.bullet_counter < 1:
            for i in range(0, 360, 15):
                sprite = SPRITE.Barrage(2, 4, th.color, i, th.rect.center)
                sprite.update()
