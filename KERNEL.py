...
class One:
    def __init__(th):
        ...
        th.menu = Menu(font, picture[0], screen, color_dict[6], (color_dict[8], color_dict[8], color_dict[8]), (120, 15))
        th.is_pause = False
        th.is_summary = False
        th.is_talk = False
        th.is_save = False
...
def save_file(name, score, total_point, flashed, flash, numbers):
    ...
    log.record_log(f'{name}_{time[0]}_{time[1]}', content)
...
def line_brick(group, pos):
    lines = sorted([(15 * randint(0, 11), choice((24, 48, 80))) for _ in range(12)], key=lambda x: x[1])
    filtered_lines = {angle: half_length for angle, half_length in lines}
    for angle, half_length in filtered_lines.items():
        start, end = coordinate(pos, angle, half_length), coordinate(pos, angle - 180, half_length)
...
class Barrage(Base):
    def __init__(th, effective, speed, angle, pos, image, group, radius=0, rotate=False, form=None):
        super().__init__(image, group, None, angle, pos, radius, rotate, form)
...
class Bullet(Base):
    def __init__(th, effective, speed, angle, pos, damage, image, group, form=None, rotate=False):
        super().__init__(image, group, None, angle, pos, rotate=rotate, form=form)
        th.effective = effective
        th.speed = speed
        th.damage = damage
...
def situation(clock):
    ...
    for info in (
        ...
        (f"{int(clock.get_fps()):9d}", (39, 50)),
        (f"{major.power:02d}  ,  {major.bullets:02d}", (39, 270)),
        (f"{two.flash:02d}", (39, 295)),
        (f"{one.combo:02d}  ,  {one.total_point:02d}", (39, 320)),
    ):
        screen.blit(blit_text(info[0], font, color), info[1])


def pause_menu():
    title = "休息ing"
    text = ("Esc 休息好了", "Del 不爬了") if one.menu.timer >= 60 else ("", "")
...
def summary_menu():
    hit = 'Hit Z Key.' if two.level <= 5 and one.menu.timer >= 60 else ''
...
def save_menu():
    title = "爬山日志"
    name = f"{f'谢谢 {log.name} 的帮助' if one.menu.timer >= 60 else ''}"
...
def full_menu(title, texts, keys, other, shortly=False):
    right = False
    one.menu.draw_menu(
        sound_cache["pick"],
        ((title, other), 8, 10, 295, right), (texts, 8, 60, 25, right), (keys, 8, 170, 50, True),
        shortly=shortly
    )


def half_menu(title, texts, interval=(0, 30, 60)):
    right = False
    one.menu.draw_menu(
        sound_cache["pick"],
        ((title,), 8, 10, 0, right), *[((texts[i],), 8, 60 + 25 * i, 0, right) for i in range(len(texts))],
        interval=interval,
        subsurface=(0, 0, 345, 110)
    )
...
keydown_start_dict = {
    ...
    pg.K_c: lambda: (setattr(one, "is_check", True), one.menu.reset_timer()) if log.total_files > 0 else None
}
...
keydown_over_dict = {
    ...
    pg.K_BACKSPACE: lambda: log.backspace_name()
}
