锐行 ~ Thunder Out of the Mountain
=

---

**这是自己做的一款小游戏**  

**在 25 年初开始做的**  

**一开始的几个月是想着做 STG 游戏的** *（没错，我是个东方众*  

**然而自己不会设计比较复杂的弹幕** *（数学和编程弱爆了*  

**所以在 25 年 7 月份就想着重做成打砖块游戏** *（看着东方灵异传就有了灵感了*  

**设定是有 3 面** *（因为懒*  

---

## 前情提要  
**“差不多深夜了，罗怎么还没回来呢？”**  

**“该不会出事了吧？”**  

**“不行，我得出去一趟！”**  

**说罢，K.璃就出门了**  

**“话说……这天气……真的……非常不让人……舒服……”**  

---

## 按键说明书  
- ### 主界面
  - 按 Z 开始爬山 *（开玩*
  - 按 C 查看日志 *（没有日志的话就当木鱼吧*
  - 按 Q Say~ Goodbye
- ### 游戏ing
  - 按 ←→ 左右移动
  - 按 Z 手动形击 *（发射子弹*
  - 按 X 切换形态 *（特殊状态*
  - 按 Space 释放形分 *（Bomb*
  - 在对话中按 Z 下一则对话
    - 也可以按 X 跳过
  - 按 Esc 休息一下
- ### 休息ing
  - 按 Esc 休息完了
  - 不想爬就按 Del 吧 *（按完就返回主界面了*
- ### 存日志
  - 按 Esc 不存
  - 按 Enter 存 *（存在用户目录的“保存的游戏”文件夹里*
  - 可以存致谢人的名称 *（其实就是玩家，最多 8 个字符*
- ### 看日志
  - 按 Esc 合上
  - 按 Del 扔掉
  - 翻页则按 ←→

---

## 说一下规则吧  
- ### K.璃
  - 形分
    - 每释放一次会消耗 12 点形力
    - 释放后会有 3 秒无敌时间
  - 形态
    - 正常形态为正常速度 *（废话*
    - 特殊形态为悬浮状态 *（键需要长按哦*
  - 形击
    - 会根据蓄力点进行连续形击 *（键需要配合长按和松开哦*
    - 每形击一次消耗 1 点蓄力
- ### 形点
  - 白色为蓄力点
    - 加蓄力
    - 最多能蓄 3 点 *（事实上其它点也能加蓄力点*
  - 蓝色为形力点
    - 加形力 *（并不是火力*
    - 最多能蓄 32 点
  - 绿色为形闪点
    - 加形闪 *（残机*
    - 不会限制点数
- ### 连击
  - 只有形力点和形闪点才能加连击 *（其实就是捡形点*
  - 拾到任何形点倒计时会重置为 2 秒
  - 倒计时结束会额外加分
- ### 分数
  - 子弹每击中目标 + 64
  - 连击倒计时结束或子站结束 + 2 ^ 连击 *（连击 > 0*
  - 结算 *（基础分 + (结束分 if 子站 == 6 else 0)*
    - 基础
      - 得点结算：得点 * 512
      - 无闪结算：连续无闪过站数 * 4096
    - 结束
      - 面数结算：面数 * 16384
      - 形力结算：形力 / 32 * 8192
- ### 砖块
  - 击落砖块会
    - 有概率生成弹幕
    - 可能掉落形点 *（每站掉落的形点数是固定的*
  - 白色砖块会有“惊喜”哦
- ### 弹幕
  - 碰到白色弹幕不会触发形闪 *（反之亦然*
  - 击落砖块生成弹幕概率为
    - 0.17 + 形力 / 1000 + *（斐波那契数*
      - 第一面：0.03
      - 第二面：0.05
      - 第三面：0.08

---

## 分发包清单
- ### CHAR.7z
  - 这是存有角色们的立绘和介绍工程文件的压缩包
  - 用解压缩程序打开会有若干个 .krz 文件，这些就是工程文件 *（画的不怎么样*
- ### DX00.pyz
  - 游戏的主程序
  - 双击就可以开始爬山啦 *（要有 Python 和 Pygame-CE 环境*
- ### LICENSE
  - 顾名思义就是许可证
  - 本游戏的程序代码就是以这个协议开源的 *（源神*
- ### README.md
  - 就是你正在阅读的

---

## 备注区
- ### 本项目版本声明
  - 本 README 所附带的游戏版本为 Ver 1.1.4
  - 如果你玩的版本不是 Ver 1.1.4
    - 版权声明请以该版本 README 为准 *（谢谢*
    - 玩法可能会有差异，所以请自行判断 *（懒*
- ### 本项目版权声明
  - 程序代码遵循 [GNU GPLv3.0](./LICENSE) 协议
  - 所有 .krz 角色立绘工程和文档文件均保留所有权利，且禁止用于 AI 训练 *（认真的*
- ### 本项目使用以下第三方资源
  - 编程语言：[Python](https://www.python.org) *（需要安装哦*
    - 推荐 3.10 - π
  - 游戏库：[Pygame-CE](https://github.com/pygame-community/pygame-ce) *（这个也是*
    - 可以在命令提示符输入 pip install pygame-ce 回车
  - 字体：[GNU Unifont](https://www.unifoundry.com/unifont) *（子集化后的，我称它为 Uni3500*
    - 子集化用到的
      - 工具：[字体子集化](http://font.ssjjss.com/font-subset)、[Fonttools](https://github.com/fonttools/fonttools)
      - 仓库：[常用汉字集](https://gitee.com/feng_xingkai/chinese)
    - 遵循 [SIL OFL 1.1](https://www.unifoundry.com/OFL-1.1.txt) 协议
  - 音频
    - 制作：[PMD](http://www5.airnet.ne.jp/kajapon/tool.html) *（喜欢 YM2203*
    - 处理：[WaveShop](https://github.com/victimofleisure/WaveShop)
  - 图像：[Krita](https://krita.org/zh-cn)
  - 打包：[Zipapp](https://docs.python.org/3/library/zipapp.html)

**本项目网址：https://github.com/An-172N/DianXian-RuiXing** *（感兴趣的话就点个星吧~*  

**(C)opyright 2026 An_172N**  